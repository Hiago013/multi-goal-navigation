...
class graph_2d():
    def __init__(self,
        ...
        self.obstacles = np.zeros((nrow, ncol), dtype=np.uint8)
        if obstacles:
            ...
            for col in range (self.ncol):
                if not self.obstacles[row, col]:
                    ...
                    for action in self.actions:
                        ...
                        if self.__check_position(next_state) and (not self.obstacles[next_state[0], next_state[1]]):
